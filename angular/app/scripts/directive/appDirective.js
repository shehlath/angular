  ...
  return{
    ...
    link: function(scope,element,attr,model){
      function validation(value){
        ...
        if(value.match('^.{3,15}$')){
          model.$setValidity('lengthE',true);
        }
        else{
          model.$setValidity('lengthE',false);
        }
        return value;
      }
...
.directive('numDirective',function(){
  return{
    ...
    link: function(scope,element,attr,model){
      function validation(value){
        if (value.match('^[0-9]+$')){
          model.$setValidity('numE',true);
        }
        else{
          model.$setValidity('numE',false);
        }
        if(value.match('^.{5,10}$')){
          model.$setValidity('lengthE',true);
        }
    ...
    link: function(scope,element,attr,model){
      function validation(value){
        if (value.match('^[A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$')){
          model.$setValidity('emailE',true);
        }
        else{
          model.$setValidity('emailE',false);
        }
        return value;
      }
      model.$parsers.push(validation);
...
.directive('genderDirective',function(){
  return{
    ...
    link: function(scope,element,attr,model){
      function validation(value){
        if (value){
          model.$setValidity('genderE',true);
        }
        else{
          model.$setValidity('genderE',false);
        }
        return value;
      }
      model.$parsers.push(validation);
